...
def get_model_pipelines() -> Dict[str, Pipeline]:
    ...
    return {
        "Lasso": _linear_pipeline(
            LassoCV(cv=_CV_SPLITS, n_alphas=50, max_iter=20000, random_state=42)
        ),
        "Ridge": _linear_pipeline(
            RidgeCV(alphas=np.logspace(-3, 3, 50), cv=_CV_SPLITS)
        ),
        "ElasticNet": _linear_pipeline(
            ElasticNetCV(cv=_CV_SPLITS, random_state=42)
        ),
        ...
        "XGBoost": Pipeline([
            _IMPUTER,
            _SCALER,
            ("model", XGBRegressor(n_estimators=100, max_depth=4, learning_rate=0.01, random_state=42)),
        ]),
    }
...
def rolling_train_predict(
    ...
    """任意 sklearn pipeline 的滚动训练预测。"""
    data = df.copy()
    data[time_col] = pd.to_datetime(data[time_col], errors="coerce")
    data = data.dropna(subset=[time_col, y_col]).copy()
    if feature_cols is None:
        feature_cols = get_predictor_cols(data)
