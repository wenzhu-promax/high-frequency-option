...
def build_calendar_predictors(
    ...
    if trade_dir_col not in out.columns:
        out = add_trade_direction_proxy(out, price_col=price_col, mid_col=mid_col, out_col=trade_dir_col,
    ...
    def _one_group(g: pd.DataFrame) -> pd.DataFrame:
        ...
        for left_sec, right_sec in spans:
            lab = _span_label(left_sec, right_sec)
        ...
        for left_sec, right_sec in spans:
            lab = _span_label(left_sec, right_sec)
            left_ns, right_ns = int(left_sec * 1e9), int(right_sec * 1e9)
            # 对每个 anchor 时刻 T，lookback 区间为 (T-right, T-left] 秒
            # start_idx[i]: 第一个 ts > T-right 的位置；end_idx[i]: 第一个 ts > T-left 的位置
            # 故 [start_idx, end_idx) 即为区间内 trade 的索引
