...
def load_trade(
    ...
    调用方: run_data_pipeline._ensure_parquets。
    ...
    trade[["underlying", "expiry", "cp_flag", "strike"]] = trade["ticker"].apply(
        parse_option_ticker,  # 定义于本模块 parse_option_ticker
    )
...
def deduplicate_by_ticker_trade_ts(df: pd.DataFrame, agg_dict: dict) -> pd.DataFrame:
    ...
    调用方: run_data_pipeline._clean（agg_dict 为 run_data_pipeline.AGG）。
...
def add_y_5s_return(
    ...
    def _one_group(g: pd.DataFrame) -> pd.DataFrame:
        """单合约分组内：对每笔成交计算 (T,T+h] 窗口内均价相对当前 mid 的收益率 y_5s_ret。"""
